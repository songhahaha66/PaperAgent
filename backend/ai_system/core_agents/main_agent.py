...
class MainAgent:
    ...
    def __init__(self, llm: BaseLanguageModel, stream_manager=None,
        ...
        from ..runtime.pipeline import pipeline_version
        ...
        if self.pipeline == "v2":
            ...
            self.system_prompt = ""
            logger.info("MainAgent 使用 v2 状态图，跳过 ReAct / ReviewAgent 初始化")
            return
    ...
    def _create_system_prompt(self) -> str:
        ...
        if self.output_mode == "word":
            system_content += (
                ...
                "4. 如果需要图表，先使用code_agent_execute生成图片，再让WriterAgent用 insert_image_to_template 插到对应章节\n"
        ...
        system_content += (
            ...
            "- inspect_document_styles: 核对当前 paper.docx 的页面、边距、标题/正文样式\n"
            "- compare_document_styles: 对照模板与成品样式，写完后必做\n"
            "- analyze_image: 识别工作区图片（封面、校徽、图表、签名章）并给出排版建议\n"
            "- analyze_docx_layout: 同时看样式档案和嵌入图片\n"
            "- review_document_appearance: 写完后验收成品样式和图片排版\n"
        ...
        if self.template_id:
            system_content += (
                ...
                f"- Word 模式必须让 WriterAgent 基于现有 paper.docx 按契约填充，不能另起一套结构\n"
                f"- 模板骨架、样式和图片清单已在上传时解析为模板契约；不要把阅读原始模板当成写作步骤\n"
                f"- 模板中的校徽、封面图、页眉装饰默认保留；新图用 insert_image_to_template 插到对应章节\n"
