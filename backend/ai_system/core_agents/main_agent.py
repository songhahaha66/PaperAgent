...
class MainAgent:
    ...
    def _create_system_prompt(self) -> str:
        ...
        system_content = (
            ...
            "- Word 模板模式写前必须看模板样式和图片：inspect_document_styles / analyze_docx_layout，不要只抽文字\n"
            "- 理解用户需求和论文目标\n\n"
            "**Phase 2: 制定写作计划并保存**\n"
        ...
        system_content += (
            ...
            "- inspect_document_styles: 看模板或成品的页面、边距、标题/正文样式\n"
        ...
        if self.template_id:
            system_content += (
                ...
                f"- Word 模式必须让 WriterAgent 先读取并识别现有 paper.docx（含图片），并基于该模板底稿生成，不能另起一套结构\n"
