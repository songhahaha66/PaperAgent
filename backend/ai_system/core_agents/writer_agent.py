        ...
        if self.output_mode == "word":
            ...
            if has_template:
                prompt = base_prompt + (
                    ...
                    "9. **insert_image_to_template** — 在锚点处插图并配图题\n"
                    "10. **extract_template_images** — 提取图片到 .system/docx_images/\n"
                    "11. **repair_template_structure** — 按模板恢复标题骨架\n"
                    "12. **create_docx / edit_docx / repack_docx** — 仅用于附件或极低级 XML 编辑\n\n"

                    "**⚠️ 核心工作流程（模板模式）**\n"
                    "1. 写前：`inspect_document_styles('.system/_template_original.docx')` 看清字体、字号、边距、对齐\n"
                    "2. 写前：`analyze_docx_layout()` 看封面/校徽/图表，不要只看文字\n"
                    "3. 文字用 `write_to_template(...)` 按位填充；图表用 `insert_image_to_template(...)`\n"
