...
from ...schemas.paper_ir import FigureRef, TableRows
...
def judge_draft(draft: Draft, slot: Slot, judge=None) -> Judgement:
    ...
    # Figures and tables are judged by presence of payload, not prose length.
    structural = any(
        (isinstance(block, FigureRef) and block.artifact_id) or (isinstance(block, TableRows) and block.rows)
        for block in draft.blocks
    )
    substantive = structural or len(text.strip()) >= max(slot.constraints.min_chars, 20)
    ...
    try:
        ...
        if "substantive" in answers:
            judgement.substantive = bool(answers["substantive"].value) or structural
