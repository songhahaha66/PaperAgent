    ...
    def create_word_tools(workspace_dir: str, stream_manager=None) -> List[StructuredTool]:
        ...
        try:
            ...
            tools = [
                ...
                StructuredTool.from_function(
                    coroutine=docx.read_docx,
                    name="read_docx",
                    description=(
                        "读取 .docx 文件的纯文本内容。"
                        "参数：filename（默认 paper.docx）。"
                        "在写作前必须先调用此工具了解现有文档内容。"
                    )
                ),
                StructuredTool.from_function(
                    coroutine=docx.edit_docx,
                    name="edit_docx",
                    description=(
                        "解包已有 .docx 文件为 XML 结构，用于精细编辑。\n"
                        "参数：operations（编辑描述）、filename（默认 paper.docx）。\n"
                        "返回解包后的文件列表和 document.xml 预览。编辑 XML 后调用 repack_docx 重新打包。"
                    )
                ),
                StructuredTool.from_function(
                    coroutine=docx.repack_docx,
                    name="repack_docx",
                    description=(
                        "将 edit_docx 解包后并编辑过的 XML 重新打包为 .docx 文件。\n"
                        "参数：filename（默认 paper.docx）。\n"
